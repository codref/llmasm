...
def render_planner_prompt(
    *,
    ...
    required = [
        ...
        "- For retrieval, computation, or file tasks, use a simple DAG: intent -> tool -> model -> final.\n"
        "  The model node MUST receive the tool's output as its input; do not wire the model node directly to the intent.\n"
        ...
        "- If you emit a tool node, its output MUST be consumed by a downstream model, compress, or final node.\n"
        "  A tool node with no outgoing edge is invalid because its result is wasted. Always wire tool -> model -> final (or tool -> final for simple pass-through).\n"
        "- Before calling a tool that needs a clean entity, insert a model node that extracts just that entity from the user prompt.\n"
        "  Examples: weather.lookup needs a city name (e.g. 'turin'), wikipedia.search needs a topic (e.g. 'stratosphere'), calculator.eval needs an expression (e.g. '2+2'), file.read needs a path.\n"
        "  Wire: intent -> extract_entity -> tool -> model -> final. The extraction model node should use input_schema RawText, output_schema RawText, and metadata.instruction asking to extract only the requested entity with no extra text.\n"
